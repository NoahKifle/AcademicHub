...
class GradesFragment : Fragment() {
    ...
    private fun populateGradesUI(assignments: List<com.example.academichub.model.AssignmentDetails>) {
        ...
        uniqueCourses.forEach { courseCode ->
            ...
            types.forEach { type ->
                ...
                val tPct = if (tPossible > 0) (tEarned / tPossible) * 100 else 0.0
    ...
    private fun setupCalculateButton() {
        binding.btnCalculate.setOnClickListener {
            val selectedCourse = binding.actCalcCourse.text.toString().trim()
            val hypotheticalPct = binding.etNewGrade.text?.toString()?.toDoubleOrNull()
            
            if (selectedCourse.isEmpty() || hypotheticalPct == null) {
                binding.tvCalcResult.visibility = View.VISIBLE
                binding.tvCalcResult.text = "Select a course and enter a grade"
                binding.tvCalcResult.setTextColor(ContextCompat.getColor(requireContext(), R.color.text_secondary))
            ...
            uniqueCourses.forEach { code ->
                val allCourseAssignments = assignments.filter { it.classCode == code }
                val doneCourseAssignments = allCourseAssignments.filter { it.isDone && it.earnedPoints.isNotEmpty() }
                
                val cPossibleDone = doneCourseAssignments.sumOf { it.points.toDoubleOrNull() ?: 0.0 }
                val cEarnedActual = doneCourseAssignments.sumOf { it.earnedPoints.toDoubleOrNull() ?: 0.0 }

                val cPossibleAll = allCourseAssignments.sumOf { it.points.toDoubleOrNull() ?: 0.0 }

                if (code.trim().equals(selectedCourse, ignoreCase = true)) {
                    val weight = if (cPossibleAll > 0) cPossibleAll else 100.0 // Default weight if no assignments exist
                    totalPossible += weight
                    totalEarned += (hypotheticalPct / 100.0) * weight
                } else {
                    // Actual data for other courses
                    totalPossible += cPossibleDone
                    totalEarned += cEarnedActual
                }
            }

            val newSemesterPct = if (totalPossible > 0) (totalEarned / totalPossible) * 100 else 0.0
            val newGpa = (newSemesterPct / 100 * 4.0)
            
            binding.tvCalcResult.visibility = View.VISIBLE
            binding.tvCalcResult.text = "Projected Semester GPA: %.2f".format(newGpa)
